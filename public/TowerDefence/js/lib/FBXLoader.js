	...
	class FBXLoader extends THREE.Loader {
		...
		load( url, onLoad, onProgress, onError ) {
			...
			loader.load( url, function ( buffer ) {
				...
				try {
					...
					onLoad( scope.parse( buffer, path, url ) );
		...
		parse( FBXBuffer, path, url = "" ) {
			...
			if ( isFbxFormatBinary( FBXBuffer ) ) {
				...
				if ( getFbxVersion( FBXText, url ) < 7000 ) {

					throw new Error( 'THREE.FBXLoader: FBX version not supported, FileVersion: ' + getFbxVersion( FBXText, url ) );
...
//					material = new THREE.MeshPhysicalMaterial();
		...
		parseParameters( materialNode, textureMap, ID ) {
			...
			if (materialNode.attrName && !materialNode.attrName.includes('Tree')) {
...
//console.log(child.relationship)
	...
	function getFbxVersion( text, path ) {
		...
		throw new Error( 'THREE.FBXLoader: Cannot find the version number for the file given.' + path );
